...
// var_dump($employer);
 ...
 $q="SELECT IFNULL(SUM(expected_value),0) IDENT  FROM  `ss_benefit`  WHERE ssid IN (SELECT ssid from strategy_statement WHERE selected >0 AND  pjid IN (SELECT pjid FROM project WHERE pj_status<>'INACTIVE') AND pjid IN (". $_REQUEST["project"] ."))";
...
$q1= "SELECT IFNULL(SUM(value),0) realized FROM `strategy_statement_savings` WHERE pjid IN (SELECT pjid FROM project WHERE pj_status<>'INACTIVE') AND pjid IN (". $_REQUEST["project"] .")";
